...
export const CustomerCreditSection: React.FC<CustomerCreditSectionProps> = ({
  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      {/* Remove Credit Confirmation */}
      {creditIdxToRemove !== null && (
        <ConfirmDialog
          isOpen
          layer="stacked"
          title="Διαγραφή Γραμμής Πίστωσης"
          message={(() => {
            const target = customerCredits[creditIdxToRemove];
            const details = [target?.customer_name, target?.amount ? formatCurrency(Number(target.amount)) : null]
              .filter(Boolean)
              .join(', ');
            return `Θέλετε να διαγράψετε αυτή τη γραμμή${details ? ` (${details})` : ''}; Η ενέργεια δεν αναιρείται.`;
          })()}
          confirmLabel="Ναι, Διαγραφή"
          onConfirm={() => handleRemoveCredit(creditIdxToRemove)}
          onCancel={() => setCreditIdxToRemove(null)}
        />
      )}
